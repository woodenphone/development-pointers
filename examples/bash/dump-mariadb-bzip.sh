...
mysqldump --defaults-extra-file=~/.my.cfg --tz-utc --quick --opt --single-transaction --skip-lock-tables --all-databases | bzip2 > ~/"all-databases.$(hostname).$(date +%Y-%m-%dT%H%M%S%z=@%s).sql.bz2"
echo "#[${0##*/}]" "Finished."
exit
##=====< Notes >=====##
